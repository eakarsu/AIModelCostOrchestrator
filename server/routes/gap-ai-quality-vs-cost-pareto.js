...
// Route: /api/gap-ai-quality-vs-cost-pareto
// Feature: Ai Quality Vs Cost Pareto
// Description: (cost vs. quality frontier)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Quality Vs Cost Pareto. (cost vs. quality frontier)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Quality Vs Cost Pareto\nDescription: (cost vs. quality frontier)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-quality-vs-cost-pareto', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-quality-vs-cost-pareto', title: 'Ai Quality Vs Cost Pareto', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-quality-vs-cost-pareto', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-quality-vs-cost-pareto']);
      return res.json({ slug: 'ai-quality-vs-cost-pareto', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-quality-vs-cost-pareto', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-quality-vs-cost-pareto', items: [], error: (e && e.message) || 'Server error' });
  }
});
