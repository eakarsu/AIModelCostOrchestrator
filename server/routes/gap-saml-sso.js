...
// Route: /api/gap-saml-sso
// Feature: Saml Sso
// Description: SAML/SSO enterprise auth
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Saml Sso. SAML/SSO enterprise auth' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'saml-sso', title: 'Saml Sso', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'saml-sso', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['saml-sso']);
      return res.json({ slug: 'saml-sso', items: r.rows || [] });
    }
    return res.json({ slug: 'saml-sso', items: [] });
  } catch (e) {
    return res.json({ slug: 'saml-sso', items: [], error: (e && e.message) || 'Server error' });
  }
});
