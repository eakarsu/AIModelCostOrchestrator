// Auto-generated feature route: budget-governor
// Domain: autonomous budget governor — Capability: predicting overages and auto-throttling
// Mounted under: /api/budget-governor
...
// POST /api/budget-governor/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a autonomous budget governor. Your task is predicting overages and auto-throttling. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/budget-governor/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/budget-governor/:id — fetch one
...
// DELETE /api/budget-governor/:id — discard a run
