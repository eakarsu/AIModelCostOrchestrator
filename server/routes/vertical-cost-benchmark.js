// Auto-generated feature route: vertical-cost-benchmark
// Domain: cost benchmarking agent — Capability: benchmarking spend vs peer cohort
// Mounted under: /api/vertical-cost-benchmark
...
// POST /api/vertical-cost-benchmark/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a cost benchmarking agent. Your task is benchmarking spend vs peer cohort. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/vertical-cost-benchmark/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/vertical-cost-benchmark/:id — fetch one
...
// DELETE /api/vertical-cost-benchmark/:id — discard a run
