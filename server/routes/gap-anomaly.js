...
// Route: /api/gap-anomaly
// Feature: Anomaly
// Description: Anomaly investigation runbook UI
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Anomaly. Anomaly investigation runbook UI' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'anomaly', title: 'Anomaly', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'anomaly', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['anomaly']);
      return res.json({ slug: 'anomaly', items: r.rows || [] });
    }
    return res.json({ slug: 'anomaly', items: [] });
  } catch (e) {
    return res.json({ slug: 'anomaly', items: [], error: (e && e.message) || 'Server error' });
  }
});
