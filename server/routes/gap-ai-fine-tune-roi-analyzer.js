...
// Route: /api/gap-ai-fine-tune-roi-analyzer
// Feature: Ai Fine Tune Roi Analyzer
// Description: /ai/fine-tune-roi-analyzer
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Fine Tune Roi Analyzer. /ai/fine-tune-roi-analyzer' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Fine Tune Roi Analyzer\nDescription: /ai/fine-tune-roi-analyzer\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-fine-tune-roi-analyzer', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-fine-tune-roi-analyzer', title: 'Ai Fine Tune Roi Analyzer', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-fine-tune-roi-analyzer', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-fine-tune-roi-analyzer']);
      return res.json({ slug: 'ai-fine-tune-roi-analyzer', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-fine-tune-roi-analyzer', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-fine-tune-roi-analyzer', items: [], error: (e && e.message) || 'Server error' });
  }
});
