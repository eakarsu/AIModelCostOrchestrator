...
// Route: /api/gap-ai-agentic-cost-governor
// Feature: Ai Agentic Cost Governor
// Description: (auto-throttle conversational agents)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Agentic Cost Governor. (auto-throttle conversational agents)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Agentic Cost Governor\nDescription: (auto-throttle conversational agents)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-agentic-cost-governor', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-agentic-cost-governor', title: 'Ai Agentic Cost Governor', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-agentic-cost-governor', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-agentic-cost-governor']);
      return res.json({ slug: 'ai-agentic-cost-governor', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-agentic-cost-governor', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-agentic-cost-governor', items: [], error: (e && e.message) || 'Server error' });
  }
});
