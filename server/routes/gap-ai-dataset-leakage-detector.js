...
// Route: /api/gap-ai-dataset-leakage-detector
// Feature: Ai Dataset Leakage Detector
// Description: (cost from inadvertent context bloat)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Ai Dataset Leakage Detector. (cost from inadvertent context bloat)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Ai Dataset Leakage Detector\nDescription: (cost from inadvertent context bloat)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['ai-dataset-leakage-detector', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'ai-dataset-leakage-detector', title: 'Ai Dataset Leakage Detector', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'ai-dataset-leakage-detector', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['ai-dataset-leakage-detector']);
      return res.json({ slug: 'ai-dataset-leakage-detector', items: r.rows || [] });
    }
    return res.json({ slug: 'ai-dataset-leakage-detector', items: [] });
  } catch (e) {
    return res.json({ slug: 'ai-dataset-leakage-detector', items: [], error: (e && e.message) || 'Server error' });
  }
});
